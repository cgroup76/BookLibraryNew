...
/**
 * Post review for a book - allowed only once and only if user read the book
 */
function postReview() {
...
/**
 * Show review form for a specific book
 * @param {number} id - Book ID to review
 */
...
/**
 * Close the review form and reset all fields
 */
...
/**
 * Handle error when rating submission fails
...
function errorRating(error) {
    if (error.status == 401) {
        ...
        Swal.fire('Error', "Could not submit your rating - please try again later", 'error');
    }
}
...
function successRating() {
    closeReviewForm();
    Swal.fire('Success', "Thank you for submitting a review for this book!", 'success');
...
/**
 * Display user's books in the UI
...
function BooksToShow(books) {
    ...
    allMyBooks.forEach((book) => {
        ...
        if (book.IsRead == 'True') {
            booksHtml += `<div class='card mb-4 book-card book-${book.Id} readTheBook read'>`;
        } else {
            booksHtml += `<div class='card mb-4 book-card book-${book.Id} '>`;
        }
...
/**
 * Handle error when loading books fails
 * @param {Object} errorMessage - Error message from AJAX call
 */
...
/**
 * Handle successful marking of book as read
 */
...
/**
 * Handle error when marking book as read fails
 * @param {Object} errorMessage - Error message from AJAX call
 */
...
/**
 * Get recommended books for user using KNN algorithm
 */
...
/**
 * Handle successful loading of recommended books
 * @param {Array} recommended - Array of recommended book objects
 */
...
/**
 * Limit the length of a string and add ellipsis if needed
...
/**
 * Handle successful loading of book reviews
